...
const Post = ({
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Pre-fetch adjacent posts
          if (postData.previous_post) {
            fetchAdjacentPost(postData.previous_post.slug);
          }
          if (postData.next_post) {
            fetchAdjacentPost(postData.next_post.slug);
          }
        } else {
  ...
  const fetchAdjacentPost = async (adjacentSlug) => {
    if (!cache.current[adjacentSlug]) {
      try {
        const response = await fetch(`${restBase}posts?slug=${adjacentSlug}&_embed=true`);
        if (response.ok) {
          const data = await response.json();
          cache.current[adjacentSlug] = data[0];
        }
      } catch (error) {
        console.error('Error fetching adjacent post data:', error);
      }
    }
  };
      ...
      if (newData.acf.screen_captures && newData.acf.screen_captures.length > 0) {
        const fetchCaptures = async () => {
          const captures = await Promise.all(
            newData.acf.screen_captures.map(async (id) => {
        ...
        <div className='section-content'>
          ...
          <div className="content-wrapper">
            ...
            <div className="work-info">
              ...
              {currentData.acf.date_complete && <p className='work-date'>{formatDate(currentData.acf.date_complete)}</p>}
              {currentData.acf.project_description && <p className='work-desc'>{currentData.acf.project_description}</p>}
              {currentData.acf.reflection && <p className='work-reflect'>{currentData.acf.reflection}</p>}
              <div className='project-links'>
                {currentData.acf.live_site && (
                  <a aria-label='Live Site Link' className='live-site-link' href={currentData.acf.live_site} target="_blank" rel="noopener noreferrer">
                ...
                {currentData.acf.github_repo && (
                  <a aria-label='Github Link' className='github-link' href={currentData.acf.github_repo} target="_blank" rel="noopener noreferrer">
          ...
          <nav className={`posts-navigation ${!hasNextPost ? 'no-next' : ''}`}>
            {hasNextPost && (
              <button aria-label='Next Works' onClick={() => onSelectWork(currentData.next_post.slug)} className="prev-post">
            ...
            {hasPrevPost && (
              <button aria-label='Previous Works' onClick={() => onSelectWork(currentData.previous_post.slug)} className="next-post">
